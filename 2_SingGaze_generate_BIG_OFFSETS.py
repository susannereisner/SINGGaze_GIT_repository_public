...
This script creates files of audio features 5 seconds around the OFFSETS of
infant social gaze, for both original and surrogate data, at a resolution of 50Hz.

Looks are only included if the audio data 5 seconds before or after the OFFSET of
infant social gaze are has <1 s silence.

Surrogate data are random timepoints in the same audio IF there is no OFFSET of
...
def gen_ERC(time_serie, look_offsets, window_size):
    #Generate "ERC" (i.e) timeseries sub-arrays for size window_size, locked on the looks contained in look_offsets
    ...
    for look in look_offsets:
        ...
        # print('tempo = ' ,tempo)
        # ref_tempo = 120  
        # # stretch the time dimension so that every ERC contains exactly 3beats before and after look offset
        # strech_ratio = ref_tempo / tempo
        #Each ERC is the timeserie value truncated around the look offset + or - the tempo-adjusted window_size
...
def get_look_list(look_file): #get the look list in the playsong and lullaby condition
    ...
    looks_pla = (dfl_pla['Offset']).to_numpy()
    looks_lul = (dfl_lul['Offset']).to_numpy()
    return looks_pla, looks_lul

def get_offset_songs(dfse,ppt_id): #get the start and end of the playsong and the lullaby
...
output_dir = '' # individual files for each ID; separate folders for offset & onset analyses
output_dir_big = '' # combined for all IDs; separate folders for offset & onset analyses
...
for ppt_id in ppt_id_list:
    ...
    start_pla,end_pla, start_lul, end_lul = get_offset_songs(df_start_end,ppt_id)
...
# generate surrogate data looks
    ...
    # plt.title("look offset Whole songs for ppt = " + ppt_id)
    # plt.xlabel("time")
...
# generation of looks dependigng on audio feature (SF, env, F0), looks & window size
...
np.save(output_dir_big+"BIG_ERCs_SF_PLA_OFF.npy", BIG_ERCs_SF_PLA[:])
np.save(output_dir_big+"BIG_ERCs_SF_SUR_PLA_OFF.npy", BIG_ERCs_SF_SUR_PLA[:])

np.save(output_dir_big+"BIG_ERCs_SF_LUL_OFF.npy", BIG_ERCs_SF_LUL[:])
np.save(output_dir_big+"BIG_ERCs_SF_SUR_LUL_OFF.npy", BIG_ERCs_SF_SUR_LUL[:])

# np.save(output_dir_big+"BIG_ERCs_SF_TOTAL_OFF.npy", BIG_ERCs_SF_TOTAL[:])
# np.save(output_dir_big+"BIG_ERCs_SF_SUR_TOTAL_OFF.npy", BIG_ERCs_SF_SUR_TOTAL[:])



np.save(output_dir_big+"BIG_ERCs_env_PLA_OFF.npy", BIG_ERCs_env_PLA)
np.save(output_dir_big+"BIG_ERCs_env_SUR_PLA_OFF.npy", BIG_ERCs_env_SUR_PLA)

np.save(output_dir_big+"BIG_ERCs_env_LUL_OFF.npy", BIG_ERCs_env_LUL)
np.save(output_dir_big+"BIG_ERCs_env_SUR_LUL_OFF.npy", BIG_ERCs_env_SUR_LUL)

# np.save(output_dir_big+"BIG_ERCs_env_TOTAL_OFF.npy", BIG_ERCs_env_TOTAL)
# np.save(output_dir_big+"BIG_ERCs_env_SUR_TOTAL_OFF.npy", BIG_ERCs_env_SUR_TOTAL)



np.save(output_dir_big+"BIG_ERCs_F0_PLA_OFF.npy", BIG_ERCs_F0_PLA)
np.save(output_dir_big+"BIG_ERCs_F0_SUR_PLA_OFF.npy", BIG_ERCs_F0_SUR_PLA)

np.save(output_dir_big+"BIG_ERCs_F0_LUL_OFF.npy", BIG_ERCs_F0_LUL)
np.save(output_dir_big+"BIG_ERCs_F0_SUR_LUL_OFF.npy", BIG_ERCs_F0_SUR_LUL)

# np.save(output_dir_big+"BIG_ERCs_F0_TOTAL_OFF.npy", BIG_ERCs_F0_TOTAL)
# np.save(output_dir_big+"BIG_ERCs_F0_SUR_TOTAL_OFF .npy", BIG_ERCs_F0_SUR_TOTAL)
